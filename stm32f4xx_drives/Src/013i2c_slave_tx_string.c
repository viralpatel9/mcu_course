...
void I2C_ApplicationEventCallback(I2C_Handle_t *pI2CHandle, uint8_t AppEv)
{
	...
	if(AppEv == I2C_EV_DATA_REQ)
	{
		...
		{
			//send the contents of the Tx_buf
	...
	else if(AppEv == I2C_ERROR_AF)
	{
		...
		//master has to sent the NACK so slave should understand that master doesnt need more data
		commandCode= 0xff;
		Cnt = 0;
	}
	else if(AppEv == I2C_EV_STOP)
	{
		// this happnes only during slave reception
